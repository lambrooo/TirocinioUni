...
def run_active_inference_simulation(n_steps=500):
    ...
    # Cyber defense is OFF initially, Agent must decide to verify/defend
    ...
    for step in range(n_steps):
        ...
        # Simulate attack (simple scenario for testing)
        if step == 200:
            print(f"Step {step}: Simulating Attack on Motor Sensor!")
            sensors['motor_temperature'].introduce_anomaly('bias', value=-30.0, duration=50) # Hide overheating

        # Get readings
        # If verifying, get verified reading (simulated as true value for simplicity here)
        # In real env, verification might cost time/money but give true value.
        # The agent's "Verify" action (Control 2) maps to env.trigger_verification
        
        # Check if agent decided to verify in previous step
        # We need to handle the "Verify" action from the *previous* plan
        # But here we plan -> act -> observe in a loop.
        # Let's assume the agent's action 2 (Verify) triggers verification NOW.
        
        # Note: The agent's plan_action returns actions to be taken.
        # We need to execute them.
        
        # Let's get the readings FIRST (Standard POMDP cycle: Observe -> Infer -> Plan -> Act)
        # But wait, observation depends on previous action? No, state depends on previous action.
        ...
        # Check for alerts (from defense system if active)
        # Here we simplify: if agent verified last step, it knows truth?
        # The agent has a "Cyber Health" state.
        # If it verifies, it might observe "Alert" if there is an attack.
        # For this bridge, let's say:
        # If sensor has anomaly AND agent verifies -> Obs Cyber = Alert
        # Else -> Obs Cyber = Safe
        
        attack_detected = False
        if env.is_verifying_sensor:
            # If verifying, we detect anomalies
        ...
        # 3. Agent Inference (VFE)
        qs = agent.infer_state(obs_indices)
        
        # 4. Agent Planning (EFE)
        actions = agent.plan_action()
        
        # 5. Bridge: Map to Controls
        ...
        # Verification
        if verify_cmd:
            # Trigger verification in env (costs money, pauses production)
            env.trigger_verification('motor_temperature') # Simplify to verifying motor
